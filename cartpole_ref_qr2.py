...
aux_loss_scaling_coef = 10 # Weight for the auxiliary quantile loss
...
def pinball_loss(prediction, target, quantile):
    """
    Calculates the pinball loss between prediction and target for a given quantile.
    """
    loss = torch.where(
        target >= prediction,
...
class QuantileActorHead(nn.Module):
    ...
    - q_high (high quantile prediction)
    ...
    def forward(self, observation: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Split the output into four equal parts along the last dimension
        # loc, log_scale, q_low, q_high
        loc, log_scale, q_low, q_high = raw_output.chunk(4, dim=-1)
        
        # Process scale: std must be positive so use exp(log_std)
        log_scale = log_scale.clamp(-20, 2)
...
def train():
    ...
    actor_net = nn.Sequential(
        ...
        #  removed NormalParamExtractor(), replace with custom extractor to handle the tensordict updates
    )

    # wrap the base network in custom head module
    custom_head_module = QuantileActorHead(actor_net)

    # wrap the custom head module in TensorDictModule
    policy_module = TensorDictModule(
        module=custom_head_module,
        in_keys=["observation"],
        # list added output keys
    ...
    policy_module = ProbabilisticActor(
        ...
        # ProbabilisticActor only uses loc and scale so keep specifying those in_keys
        # "q_low" and "q_high" will be passed through the tensordict automatically.
        in_keys=["loc", "scale"],
    ...
    for i, tensordict_data in enumerate(collector):
        for epoch in range(num_epochs):
            ...
            for _ in range(frames_per_batch // sub_batch_size):
                ...
                ### MODIFIED ###
                # Detach the network's predicted action, this is now the target for the aux loss / quantile heads
                ...
                aux_loss = (loss_q_low + loss_q_high) * aux_loss_scaling_coef
...
def eval():
    ...
    with set_exploration_type(ExplorationType.DETERMINISTIC), torch.no_grad():
        while i < max_ep_steps and not done:
            ...
            # We only need to assign the 'action' key for the env step
    ...
    rollout = rollout[:i] # Trim the rollout to the actual length since we preallocated all the space
    ...
    # --- Actions, Observations, and Quantiles ---
    ...
    # shade the area between quantiles
    plt.fill_between(
        steps,
        q_low_np,
        q_high_np,

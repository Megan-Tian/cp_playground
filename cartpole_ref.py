...
def train():
    ...
    eval_str = ""
    writer = SummaryWriter(f"runs/cartpole_ref_pytorch_ppo_{total_frames}")
    ...
    torch.save(policy_module, f'baseline/cartpole-policy-{total_frames}.pth')
    print(f'saved policy!')
    torch.save(logs, f'baseline/cartpole-train-val-logs-{total_frames}.pkl')
...
def eval():
    ################## LOAD POLICY ##################
    policy_path = f"baseline/cartpole-policy-{total_frames}.pth"
    ...
    # logger = CSVLogger(exp_name="cartpole", log_dir="cartpole_videos", video_format="mp4")
    ...
    utils.video_writer(rollout["pixels"], f"cartpole_videos/cartpole/videos/pendulum-video-{datetime.now()}.mp4")

    ################## PLOT RESULTS ##################
    rewards = rollout["next", "reward"]
    actions = rollout["action"]
    observations = rollout["next", "observation"]
    ...
    plt.plot(actions.detach().numpy(), label="action")
    plt.plot(observations.detach().numpy(), label="observation")
